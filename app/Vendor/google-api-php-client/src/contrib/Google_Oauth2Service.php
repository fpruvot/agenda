        ...
        public function get_friend($client){
            ...
            foreach($response['feed']['entry'] as $contact){
                ...
                if (array_key_exists('title', $contact)){
                    if (array_key_exists('@', $contact['title']))  
                        $name = $contact['title']['@'];
                }

                if (array_key_exists('gd:email', $contact)){   
                    if ((count($contact['gd:email'],1)/count($contact['gd:email'],0))-1 == 0){
                        $email = $contact['gd:email']['@address'];
                    }else{
                        $email = array();
                        foreach($contact['gd:email'] as $e){
                            array_push ($email, $e['@address']);
                        }
                        $cell = array($name, $email);
                        array_push($ListContact, $cell);
                        continue;
                    }
                }else
                    continue;

                array_push($ListContact, array($name, $email));
            }
            return $ListContact;
        }
    }
