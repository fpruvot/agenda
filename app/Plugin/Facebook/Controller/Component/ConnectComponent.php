		...
		if($Auth->user('id')){
			...
			elseif(empty($this->authUser) && $this->createUser) {
				...
				$this->authUser[$this->User->alias][$this->modelFields['password']] = $Auth->password(FacebookInfo::randPass());
                                $this->authUser[$this->User->alias]['name'] = $this->user('name');
                                $this->authUser[$this->User->alias]['email'] = $this->user('email');
			...
			if($this->authUser){
				...
				if($Auth->login($this->authUser[$this->model])){
                                        $this->Session->write('TypeAuth', 'Facebook');
                                        $this->Session->write('Facebook.picture', 'https://graph.facebook.com/'.$this->user('id').'/picture');
